...
- **SessionStart hook** — branches on `.atlas-repo/.git` existence:
  - **Healthy state** (`scripts/core/atlas-sync.sh`) — pulls the latest Atlas (depth 20 so `process-pr.sh` can diff against parent) and rebuilds `data/index.json` + address map. This is the only thing that touches `.atlas-repo/`; Claude's sandbox denies writes there. **Staleness-gated auto-refresh:** it also checks `.last-session`'s mtime (marked at the end of every `/refresh`) and, when the caches are >6h stale (or never refreshed this clone), emits an `additionalContext` directive telling the session to proactively run `/refresh` on its first turn. A fresh re-open (<6h) stays quiet. The hook can't run `/refresh` itself — the fetches are slow and the `/atlas-track` Context-fill needs an agent turn — so it drives the agent rather than shelling out. Tune the window via `STALE_AFTER` in the script.
...
- **`/refresh` skill** (`scripts/core/refresh.sh`) — user-invoked, and auto-invoked on the session's first turn when the SessionStart hook flags the caches as >6h stale (above). Refreshes all data caches (voting, forum, delegates, market, financials, open PRs), auto-processes any unprocessed merged PRs into `history/`, then prints the session briefing. Errors with a setup hint if `data/index.json` is missing.
...
3. `enrich.py` — poll/spell lookup, governance-flow classification (1/2/3), **UUID-anchored** routing via `entity-routing.conf` (each rule carries the immutable UUID of its subtree root; a doc routes by matching that anchor against its own/ancestor UUIDs, so routing survives renumbering — number prefixes are only a fallback; NR docs route by their `targets[0]` parent). Also flags reference-renumber-only docs and warns if a routing anchor is missing from the index.
4. `render.py` — template-based markdown emission per entity; addition trees, deletion bullets, modification bullets, sweep groupings. **Reference-renumber noise** (dotted cross-reference path segments that shifted because a *linked* doc moved — e.g. `…2.5.1` → `…2.6.1`, UUID target unchanged) is collapsed into one `Reference renumbering across N docs` housekeeping line rather than per-doc bullets. The `### Context` section is emitted as a `<!-- context: pending -->` placeholder — **except** for pure-propagation entries (no material changes, only the renumber-summary line), which are auto-filled with a factual one-liner and need no human Context.
...
**Context paragraphs are filled by the session agent, not the pipeline.** After `process-pr.sh` returns, the `/atlas-track` skill replaces each `<!-- context: pending -->` placeholder with 1-2 sentences using the surrounding session's full context (or strips the section if there's nothing notable). `/refresh` delegates this to `/atlas-track` automatically for newly-processed PRs. Pure-propagation entries (the renumber-only case above) arrive already auto-contextualized — they have **no** pending placeholder, so `/atlas-track` skips them; only entries with a real material change need human Context. This is deliberate — an earlier design shelled out to a nested `claude -p` from inside the pipeline, which broke any time `process-pr.sh` was invoked from within a Claude Code session (and that's the only way this project is ever invoked).
...
Changes are tracked per-scope in `history/` with changelogs routed by `entity-routing.conf`. Each rule is `prefix<TAB>dir<TAB>anchor-uuid`; routing matches the **immutable anchor UUID** against a changed doc's own/ancestor UUIDs (renumber-proof), falling back to the most-specific number prefix. (This caught a live bug: the ad-roster and ad-breaches subtrees were renumbered `A.1.5.x → A.1.6.x` upstream, silently breaking number-prefix routing until the UUID anchors fixed it.) Agents have their own subdirectories under `A.6--agents/`. Article-level splits within a scope follow the same pattern (e.g., `A.2--support/A.2.7.1.1.1.1--forum-accounts/` for the Authorized Forum Accounts registry, `A.1--governance/A.1.5.6.1.3--ad-breaches/` for the AD Breach Registry — both Active Data nodes that mutate between weekly cycles via Flow 3 Direct Edit). The directory may be incomplete — new agents and article splits can be added via governance at any time.
